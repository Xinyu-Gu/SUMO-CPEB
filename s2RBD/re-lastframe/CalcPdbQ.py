...
def computeQ():
	...
	norm = 0
        for ia in range(0, 6):
                for ja in range(0, M-6):
                                r = vabs(vector(ca_atoms_pdb2[ia+33], ca_atoms_pdb2[ja+267+15]))
                                rn = vabs(vector(ca_atoms_pdb[ia+M-6], ca_atoms_pdb[ja]))
                                if qo_flag == 1 and rn >= 12 and r >= 12: continue
                                dr = r - rn
                                Q = Q + exp(-dr*dr/10)
                                norm = norm + 1
	Q = Q/norm
