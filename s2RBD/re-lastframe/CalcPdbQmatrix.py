...
def computeQ():
	...
	min_sep = 3
        for ia in range(0, 100):
                for ja in range(max(100, ia+min_sep), 267):
#	for ia in range(0, N):
#		for ja in range(ia+min_sep, N):
        ...
        for ia in range(100, 200):
                for ja in range(max(200, ia+min_sep), 267):
                                r = vabs(vector(ca_atoms_pdb[ia], ca_atoms_pdb[ja]))
                                rn = vabs(vector(ca_atoms_pdb2[ia], ca_atoms_pdb2[ja]))
                                if qo_flag == 1 and rn >= 9.5 and r >=9.5: continue
                                dr = r - rn
        ...
        for ia in range(0, 267):
                for ja in range(267, N):
                                r = vabs(vector(ca_atoms_pdb[ia], ca_atoms_pdb[ja]))
                                rn = vabs(vector(ca_atoms_pdb2[ia], ca_atoms_pdb2[ja]))
                                if qo_flag == 1 and rn >= 9.5 and r >=9.5: continue
                                dr = r - rn
...
Nframe=[6, 9, 16, 18, 21, 23, 25, 28, 29, 33, 43, 48, 52, 56, 60]
