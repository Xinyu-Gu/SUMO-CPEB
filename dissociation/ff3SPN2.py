...
class DNA(object):
    ...
    def __init__(self, periodic=True, k_3spn2=1):
...
class Bond(Force, simtk.openmm.CustomBondForce):
    ...
    def reset(self):
        ...
        bondForce = simtk.openmm.CustomBondForce(f"{k_3spn2}*(Kb2*(r-r0)^2+Kb3*(r-r0)^3+Kb4*(r-r0)^4)")
...
class Stacking(Force, simtk.openmm.CustomCompoundBondForce):
    ...
    def reset(self):
        k_3spn2 = self.k_3spn2
        stackingForce = simtk.openmm.CustomCompoundBondForce(3, f"""{k_3spn2}*(rep+f2*attr);
...
class Dihedral(Force, simtk.openmm.CustomTorsionForce):
    ...
    def reset(self):
        k_3spn2 = self.k_3spn2
        dihedralForce = simtk.openmm.CustomTorsionForce(f"""{k_3spn2}*(K_periodic*(1-cs)-K_gaussian*exp(-atan(tan(dt))^2/2/sigma^2));
                                                      cs=cos(dt);
                                                      dt=theta-t0""")
...
class BasePair(Force, simtk.openmm.CustomHbondForce):
    ...
    def reset(self):
        def basePairForce():
            k_3spn2 = self.k_3spn2
            pairForce = simtk.openmm.CustomHbondForce(f'''{k_3spn2}*temp;temp=rep+1/2*(1+cos(dphi))*fdt1*fdt2*attr;
    ...
    def reset(self):
        def crossStackingForce(parametersOnDonor=False):
            if self.OpenCLPatch:
                ...
                patch_string = ''
            k_3spn2 = self.k_3spn2
            crossForce = simtk.openmm.CustomHbondForce(f'''{k_3spn2}*energy{patch_string};temp=dtCS/rng_CS;
...
class Exclusion(Force, simtk.openmm.CustomNonbondedForce):
    ...
    def reset(self):
        k_3spn2 = self.k_3spn2
        exclusionForce = simtk.openmm.CustomNonbondedForce(f"""{k_3spn2}*energy;
...
class Electrostatics(Force, simtk.openmm.CustomNonbondedForce):
    ...
    def reset(self):
        ...
        denominator = 1/0.415*(unit.kilocalorie_per_mole**-1 * unit.nanometer**-1)
        print(ldby, denominator)
        k_3spn2 = self.k_3spn2
        electrostaticForce = simtk.openmm.CustomNonbondedForce(f"""{k_3spn2}*energy;
    ...
    def defineInteraction(self):
        ...
        for i, atom in atoms.iterrows():
            if atom.is_dna:
                ...
                if atom['resSeq'] == 1 and atom['name'] == 'S':
                   parameters = [-0.6]
                print(atom.chainID, atom.resSeq, atom.resname, atom['name'], parameters)
            else:
...
class PinDNA(Force, simtk.openmm.CustomExternalForce):

    def __init__(self, dna, chain_DNA='B', k_pin=400*4.184, pin_file="./pin_DNA.dat"):
    ...
    def reset(self):
        k_pin = self.k_pin
        pinForce = simtk.openmm.CustomExternalForce(f"{k_pin}*((x-x0)^2+(y-y0)^2+(z-z0)^2)")
        pinForce.addPerParticleParameter("x0")
        pinForce.addPerParticleParameter("y0")
        pinForce.addPerParticleParameter("z0")
        self.force = pinForce
        print("PinDNA on, k_pin=", k_pin)

    def defineInteraction(self):
        atoms = self.dna.atoms.copy()
        atoms['index'] = atoms.index

        pins=np.loadtxt(self.pin_file)
        for i in range(len(pins)):
            S_atoms = atoms[(atoms['chainID']==self.chain_DNA) & (atoms['name'].isin(['S'])) & (atoms['resSeq']==int(pins[i][0])) & atoms['resname'].isin(_dnaResidues)].copy()
            x0 = pins[i][1]/10*unit.nanometer
            y0 = pins[i][2]/10*unit.nanometer
            z0 = pins[i][3]/10*unit.nanometer
            self.force.addParticle(int(S_atoms['index'].values[0]), [x0, y0, z0])
            print(int(S_atoms['index'].values[0]), [x0, y0, z0])
...
class ElectrostaticsProteinDNA(ProteinDNAForce):
    ...
    def __init__(self, dna, protein, coord_ion_file=None, force_group=15):
        self.force_group = force_group
        self.coord_ion_file = coord_ion_file
        super().__init__(dna, protein)
    ...
    def reset(self):
        ...
        ldby = 1.0 * unit.nanometer # np.sqrt(dielectric * pv * kb * T / (2.0 * Na * ec ** 2 * C))
        #denominator = 4 * np.pi * pv * dielectric / (Na * ec ** 2)
        #denominator = denominator.in_units_of(unit.kilocalorie_per_mole**-1 * unit.nanometer**-1)
        denominator = 0.6/0.415*(unit.kilocalorie_per_mole**-1 * unit.nanometer**-1)
        print(ldby, denominator)
        k_3spn2 = self.k_3spn2
        electrostaticForce = simtk.openmm.CustomNonbondedForce(f"""{k_3spn2}*energy;
    ...
    def defineInteraction(self):
        ...
        if self.coord_ion_file:
            ionInfo = np.loadtxt(self.coord_ion_file)
        tmp = -1
        resid = 0
        ...
        for i, atom in atoms.iterrows():
            if atom.is_dna:
                ...
                if self.coord_ion_file and (resid in ionInfo[:,1]) and (atom['name']=='CB'): 
                   for jndex in range(len(ionInfo)):
                       if ionInfo[jndex][1] == resid: charge = ionInfo[jndex][0]
                parameters = [charge]
                if charge != 0:
                    protein_list += [i]
                    print(atom.chainID, atom.resSeq, atom.resname, atom['name'], charge)
...
class AMHgoProteinDNA(ProteinDNAForce):
    ...
    def __init__(self, dna, protein, chain_protein='A', chain_DNA='B', k_amhgo_PD=1*unit.kilocalorie_per_mole, sigma_sq=0.05*unit.nanometers**2, aaweight=False, localct=False, cutoff=0.45, force_group=16):
        self.force_group = force_group
        ...
        self.cutoff = cutoff
        self.localct = localct
        super().__init__(dna, protein)

    def reset(self):
        cutoff = self.cutoff
        k_3spn2 = self.k_3spn2
        k_amhgo_PD = k_3spn2*self.k_amhgo_PD.value_in_unit(unit.kilojoule_per_mole)
        sigma_sq = self.sigma_sq.value_in_unit((unit.nanometers)**2)
        if self.localct:
                amhgoForce = simtk.openmm.CustomBondForce(f"-{k_amhgo_PD}*gamma_ij*exp(-(r-r_ijN)^2/(2*{sigma_sq}))*step(r_ijN+{cutoff}-r)")
        else:
                amhgoForce = simtk.openmm.CustomBondForce(f"-{k_amhgo_PD}*gamma_ij*exp(-(r-r_ijN)^2/(2*{sigma_sq}))")
...
class StringProteinDNA(ProteinDNAForce):
    ...
    def __init__(self, dna, protein, r0, chain_protein=['A'], chain_DNA=['B'], k_string_PD=10*4.184, prot_seg=False, prot_group=[], DNA_seg=False, DNA_group=[]):
        self.k_string_PD = k_string_PD
        self.chain_protein = chain_protein
        self.chain_DNA = chain_DNA
        self.r0 = r0
        self.prot_seg = prot_seg
        self.prot_group = prot_group
        self.DNA_seg = DNA_seg
        self.DNA_group = DNA_group
        super().__init__(dna, protein)
    ...
    def defineInteraction(self):
        atoms = self.dna.atoms.copy()
        atoms['index'] = atoms.index
        CA_atoms = atoms[(atoms['chainID'].isin(self.chain_protein)) & (atoms['name'] == 'CA') & atoms['resname'].isin(_proteinResidues)].copy()
        S_atoms = atoms[(atoms['chainID'].isin(self.chain_DNA)) & (atoms['name'] == 'S') & atoms['resname'].isin(_dnaResidues)].copy()
        CA_index = [int(atom.index) for atom in CA_atoms.itertuples()]
        S_index = [int(atom.index) for atom in S_atoms.itertuples()]
        if self.prot_seg: self.force.addGroup([CA_index[x] for x in self.prot_group])     
        else:   self.force.addGroup(CA_index)
        if self.DNA_seg: self.force.addGroup([S_index[x] for x in self.DNA_group])
        else:   self.force.addGroup(S_index)
        bondGroups=[]
        bondGroups.append(0)
        bondGroups.append(1)
...
class String_length_ProteinDNA(ProteinDNAForce):
    """ Protein-DNA string potential"""
    def __init__(self, dna, protein, chain_protein=['A'], chain_DNA=['B'], prot_seg=False, prot_group=[], DNA_seg=False, DNA_group=[], force_group=17):
        self.force_group = force_group
        self.chain_protein = chain_protein
        self.chain_DNA = chain_DNA
        self.prot_seg = prot_seg
...
class BiasProteinDNAamhgo(ProteinDNAForce):
    """ Protein-DNA string potential"""
    def __init__(self, dna, protein, E0, k_string_PDamhgo=1, chain_protein='A', chain_DNA='B', k_amhgo_PD=1*unit.kilocalorie_per_mole, sigma_sq=0.05*unit.nanometers**2, aaweight=False, localct=False, cutoff=0.45):
        self.k_string_PDamhgo = k_string_PDamhgo
        self.E0 = E0
        self.k_amhgo_PD = k_amhgo_PD
        self.sigma_sq= sigma_sq
        self.chain_protein = chain_protein
    ...
    def reset(self):
        E0=self.E0.value_in_unit(unit.kilojoule_per_mole)
        k_string_PDamhgo=self.k_string_PDamhgo.value_in_unit((unit.kilojoule_per_mole)**(-1))
        stringForce = simtk.openmm.CustomCVForce(f"0.5*{k_string_PDamhgo}*(E_PDamhgo-({E0}))^2")
        tmp_amhgo = AMHgoProteinDNA(self.dna, self.protein, chain_protein=self.chain_protein, chain_DNA=self.chain_DNA, k_amhgo_PD=self.k_amhgo_PD, sigma_sq=self.sigma_sq, aaweight=self.aaweight, localct=self.localct, cutoff=self.cutoff)
        E_PDamhgo = tmp_amhgo.force
        stringForce.addCollectiveVariable("E_PDamhgo", E_PDamhgo)
        self.force = stringForce

    def defineInteraction(self):
        print("String_PDamhgo bias on: E0, k_string = ", self.E0, self.k_string_PDamhgo)
...
protein_dna_forces=dict(ExclusionProteinDNA=ExclusionProteinDNA,
                        ...
                        AMHgoProteinDNA=AMHgoProteinDNA,
                        String_length_ProteinDNA=String_length_ProteinDNA,
                        StringProteinDNA=StringProteinDNA,
                        BiasProteinDNAamhgo=BiasProteinDNAamhgo)
