...
cluster = hierarchy.fcluster(row_linkage, t=0.6, criterion='distance')
...
C = hierarchy.dendrogram(row_linkage, truncate_mode='lastp', p=15, no_labels=True, color_threshold=0.4)
plt.savefig('cluster_size.png')
plt.close()

T=sns.clustermap(qw,col_linkage=row_linkage,row_linkage=row_linkage)
plt.savefig('cluster_qw_interD.png')
